...
degraded run이면 "none (...)"), 네 task ID, 그리고 누구에게 보고할지를 준다.

## 0단계 — 입력부터 읽어라

분석에 들어가기 전에 모두 `Read`하라:

```
{MARKET_DATA}                              # 있다면 — 저장소 @tool 출력 (OHLCV·지표·거래량)
{OUTPUT_DIR}/01-technical-analysis.md
{OUTPUT_DIR}/02-fundamentals-analysis.md
{OUTPUT_DIR}/03-news-sentiment-analysis.md
```

01–03 중 하나라도 **없거나 비어 있으면** 멈추고 어느 것인지 밝혀 디스패처에게
`SendMessage`하라 — 너무 일찍 생성된 것이다. 부분 입력만으로 진행하지 말고 빠진
부분을 직접 조사하지도 마라. 그것은 다른 에이전트의 task이며 중복하면 상충하는
두 번째 출처가 생긴다.

`00-market-data.md`가 이 런에서 정확한 가격·지표·거래량 수치의 **source of truth**다.
세 리포트와 어긋나면 00이 이긴다 (단, `STALE OHLCV WARNING`이 있으면 그 사실을
신뢰도에 반영하라). 00에는 일자별 거래량과 지표 히스토리가 있으니, 변동성·유동성
등급과 손절 폭(ATR 배수)은 추정하지 말고 그 값으로 산정하라.
...
- **진입가, 목표가, 손절가는 00의 verified close와 기술적 리포트의 실제 레벨에
  앵커링돼야 한다.** 네가 편하게 느끼는 반올림 숫자가 아니다. 가격 산술(변동률, 레벨 간
  거리, ATR 배수)은 00 수치로 계산하고 검산하라. `{PRICE}`가 "unknown"이었다면
  레벨이 상대적이며 앵커가 없다고 밝혀라.
- **손익비는 네 진입가, 목표가, 손절가와 산술적으로 일관돼야 한다.** 00의 verified
  close 기준으로 확인하라.
