...
이 구조의 목적은 종목 분석을 파일 단위로 재현하는 것이다. 팀 정의, 에이전트, 스킬,
워크플로, 팀런 산출물이 모두 `.claude` 안에서 연결된다.
...
| 계약 | 의미 |
|---|---|
| 저장 위치 | 모든 산출물은 `.claude/team-runs/{DATE}-{TICKER}/`에 저장한다. |
...
현재 `.claude`는 TradingAgents 포크에서 트레이딩 분석을 교육용으로 재현하기 위한 실행팀이다.
계산은 저장소 툴로 검증하고,
